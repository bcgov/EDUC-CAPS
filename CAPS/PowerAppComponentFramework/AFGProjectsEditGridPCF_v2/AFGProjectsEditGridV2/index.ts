...
export class AFGProjectsEditGridV2 implements ComponentFramework.StandardControl<IInputs, IOutputs> {
    ...
    public updateView(context: ComponentFramework.Context<IInputs>): void
    {
		...
		if(!context.parameters.AFGProjectsDataSet.loading){
			...
			try {
				const lockInactive:string = context.parameters.lockoninactivestate.raw as string;
            ...
            console.log("submission: "+ (this.contextObj.mode as any).contextInfo.entityId);
	...
	private createGridBody(context: ComponentFramework.Context<IInputs>, columnsOnView: DataSetInterfaces.Column[], gridParam: DataSet):HTMLDivElement{
		...
		const rankingFieldName:string = 'bingo'; //context.parameters.rankingfieldname.raw as string;
		const groupingFieldName:string = 'bingo'; //context.parameters.groupingfieldname.raw as string;
		const linkfieldname:string = context.parameters.linkfieldname.raw as string;
		const entitydisplayname:string = context.parameters.entitydisplayname.raw as string;
		...
		const columnHeader:HTMLDivElement = this.getColumnHeaders(columnsOnView, groupingFieldName);

		const categoryHeader:HTMLDivElement = this.getCategoryHeader('', columnHeader);
		...
		if (gridParam.sortedRecordIds.length > 0)
		{
			for(const currentRecordId of gridParam.sortedRecordIds){
				const record:DataSetInterfaces.EntityRecord = gridParam.records[currentRecordId];
				...
				const linkValue:string = gridParam.records[currentRecordId].getFormattedValue(linkfieldname);
				...
				columnsOnView.forEach(function(columnItem, index){
					...
					if (!columnItem.isHidden &&  columnItem.name.indexOf(groupingFieldName) == -1) {
						const valuePara:HTMLDivElement = document.createElement("div");
						...
						const fieldName:string = columnItem.name.toLowerCase();
				...
				const allowUnlinkRecord:string = this.contextObj.parameters.allowunlinkrecord.raw as string;
	...
	private getAddEditRow(context:ComponentFramework.Context<IInputs>, columnsOnView: DataSetInterfaces.Column[], entityDisplayName:string, isAddRow:boolean, row:HTMLDivElement|null):HTMLDivElement {
		...
		const obj = this;
		let recordId:string = '';
        let dataFields: JQuery<HTMLDivElement>;
				...
				if (columnItem.name.indexOf('caps_projectnumber') > -1 || columnItem.name.indexOf('caps_projectcode') > -1) {
					...
					switch(columnItem.dataType.toLowerCase())
					{
						...
						case 'twooptions': {
							...
							input.setAttribute('data-datatype', columnItem.dataType);
							if (!isAddRow) {
								const val:string = $(dataFields[index-hiddenCount]).data('value').toLowerCase(); 
	...
	private getCategoryHeader(categoryName:string, columnHeader:HTMLDivElement):HTMLDivElement {
		const categoryHeader:HTMLDivElement = document.createElement("div");
	...
	private getColumnHeaders(columnsOnView: DataSetInterfaces.Column[], groupingFieldName:string):HTMLDivElement {
		const columnHeader:HTMLDivElement = document.createElement("div");
		columnHeader.classList.add("DataSetControl_grid-header");

		columnsOnView.forEach(function(columnItem, index){
			if (!columnItem.isHidden && columnItem.name != groupingFieldName) {
				const labelPara:HTMLDivElement = document.createElement("div");
	...
	private onRowClick(event: Event): void {
		const rowRecordId = (event.currentTarget! as HTMLTableRowElement).parentElement!.getAttribute(RowRecordId);
	...
	private bindEvents() {
		...
		$('.add-button').on('click', function(event) {
			...
			const fields = div.find('.input-field');
			const entity:ComponentFramework.WebApi.Entity = {};

			const recordId = div.attr('recordId');
					...
					if (response.entities != null && response.entities.length > 0) {						
						...
						fields.each(function() {
							...
							if (required && $(this).val() == '' && $(this).prop('disabled') === false) {
								const fieldDisplayName:string = $(this).data('fielddisplayname');
						...
						if (valid==true) {
							...
							const alertStrings = { confirmButtonLabel: "Ok", text: validationErrors, title: "Validation Errors:" };
		...
		$(".edit-button").on('click', function(event) {
			...
			row.hide();
			const div:HTMLDivElement = obj.getAddEditRow(obj.contextObj, obj.getSortedColumnsOnView(obj.contextObj), entityDisplayName, false, <HTMLDivElement>row[0]);
			$(div).insertAfter(row);
